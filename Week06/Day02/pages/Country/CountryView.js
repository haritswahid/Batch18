...
export default function Batch() {
    ...
    useEffect(() => {
        setPageNumbers(Array.from({ length: Math.ceil(listCountry.length / dataPerPage) }, (v, i) => (i + 1 === 1 ? { number: i + 1, active: true } : { number: i + 1, active: false })))
        setCurrentPage(1)
        setPageRange(0)
    }, [listCountry])
                        ...
                        <div className="align-middle inline-block min-w-full border-b border-gray-200 ">
                            <table className="min-w-full">
                                ...
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {Array.isArray(listCountry) && listCountry.slice((currentPage - 1) * dataPerPage, currentPage * dataPerPage).map((data) => (
                                ...
                                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div>
                                        <p className="text-sm text-gray-700">
                                            Showing <span className="font-medium">{(currentPage - 1) * dataPerPage + 1}</span> to <span className="font-medium">{(currentPage) * dataPerPage < listCountry.length ? (currentPage) * dataPerPage : listCountry.length}</span> of{' '}
